import React,{ PureComponent,useState,useEffect } from 'react'
import {Table} from 'react-bootstrap';
import {
  ResponsiveContainer,
 BarChart,
  Bar,
  XAxis,
  YAxis,
  CartesianGrid,
  Tooltip,
  Legend,
} from 'recharts';
const App = () => {
const [data,setData] = useState([]);
const [country,setCountry] = useState([]);
const [search,setSearch] = useState('');
const [filter,setFilter] = useState("");

useEffect(()=>{
  var myHeaders = new Headers();
myHeaders.append("Cookie", "BIGipServerdataapi.sfarm=1604433930.20480.0000; TS019266c8=017189f9479a7ea7b5c64d7a4c29b5c7032e5011cfc60342b446a6bcd57b9a547a06a79741df5e8e10d29e7f065b8815368f51d4abdd8b145098492072f9377eaa4442cda2; TS019865bf=; TS01b02907=01ebd1be3b129d36386c40f0002024b9c1021ba89cb7f9bf18bd4fc67813437aafffc6c5ed93a288903fe3d83a6cdd624a43c827cc; TS01fa65e4=01359ee9768ca51c5659b484820776bca317c55efccd622b9717d3102d8160a20059cc78d359472f6bdcf942a3c139e4ec76e4535208dd70766b3b655d3e4a5b6fe44ec9b4; api_https.cookie=286e43d9dbb589273e07f096cd441f82; api_https.cookieCORS=286e43d9dbb589273e07f096cd441f82; dataapi.cookie=853586186.20480.0000");

var requestOptions = {
  method: 'GET',
  headers: myHeaders,
  redirect: 'follow'
};
fetch("https://api.worldbank.org/v2/country/indicator/SP.POP.TOTL?format=json&date=2021&per_page=300", requestOptions)
.then(response => response.json())
.then(result => {
  setData(result[1].map(item=>item.value));
  setCountry(result[1].map(item=>item.country.value));
 
}
)
},[])

const handleSearch = (e) => {
  setSearch(e.target.value)
  const results = country.filter(country => country.toLowerCase().includes(e.target.value.toLowerCase()))
  setFilter(results)
}



  


  
 
  return (
   <>
   <div>
   
    <input type="text" value={search} onChange={handleSearch}/>
    
    </div>

    
   
  
   <Table>
    <thead>
      <tr>
        <th>Country</th>
        <th>Population</th>
      </tr>
    </thead>
    <tbody>
      {
        data.map((item,index)=>{
          return(
            <tr key={index}>
              <td>{
                filter.length > 0 ? filter[index] : country[index]
              }</td>
              <td>{Number(data[index]).toLocaleString('en-US', {maximumFractionDigits:2})}</td>
            </tr>
          )
        }
        )

      }

    </tbody>


   </Table>
    


   </>
 
  )
}

export default App